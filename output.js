//Sat Feb 28 2026 03:18:39 GMT+0000 (Coordinated Universal Time)
//Base:<url id="cv1cref6o68qmpt26ol0" type="url" status="parsed" title="GitHub - echo094/decode-js: JS混淆代码的AST分析工具 AST analysis tool for obfuscated JS code" wc="2165">https://github.com/echo094/decode-js</url>
//Modify:<url id="cv1cref6o68qmpt26olg" type="url" status="parsed" title="GitHub - smallfawn/decode_action: 世界上本来不存在加密，加密的人多了，也便成就了解密" wc="741">https://github.com/smallfawn/decode_action</url>
const $ = new Env("91");
const CryptoJS = createCryptoJS();
let key = CryptoJS.enc.Utf8.parse("cc88ddc9357ff461e08f047aedee692b");
let iv = CryptoJS.enc.Utf8.parse("e89225cfbbimgkcu");
let body = $response.body;
let data = JSON.parse(body);
let url = "undefined" !== typeof $request ? $request.url : "";
let json = JSON.parse(decryptAES_CFB(data.data, key, iv));
if (json.data?.["info"]) {
  json.data.info.nickname = "联合国儿童基金会";
  json.data.info.isVip = true;
  json.data.info.isLogin = true;
  json.data.info.canWatchCount = "9999";
  json.data.info.watchStr = "无限看";
  json.data.info.coins = 9999;
  json.data.info.is_login = true;
  json.data.info.vip_level = 4;
  json.data.info.level = 9;
  json.data.info.exp = 2741;
  json.data.info.buy_count = 2;
  json.data.info.vip_level_icon = "91终身成就卡";
  json.data.info.vip_icon = "91终身成就卡";
  json.data.info.role_id = 8;
  $.log("解锁会员");
} else {
  if (json.data?.["privilege"]) {
    json.data.is_vip = true;
    json.data.vip_level = 4;
    json.data.nickname = "联合国儿童基金会";
    json.data.vip_str = "91终身成就卡";
    $.log("解锁会员中心");
  } else {
    if (json.data?.["versions"]) {
      json.data.start_screen_ads = [{}];
      delete json.data.screen;
      delete json.data.home_pop_ads;
      delete json.data.pop_ads;
      delete json.data.buoy;
      json.data.isVip = true;
      json.data.vip_level = 4;
      json.data.dark_vip_tips = "";
      json.data.recommendedData.forEach(_0x39e43f => _0x39e43f.coins = 0);
      json.data.recommendedData.forEach(_0x161fa5 => _0x161fa5.hasBuy = true);
      json.data.recommendedData.forEach(_0x37493b => {
        typeof _0x37493b.playUrl === "string" && (_0x37493b.playUrl = _0x37493b.playUrl.replace(/\/\/[^.]+/, "//long"));
      });
      $.log("去除开屏广告");
    } else {
      if (json.data?.[0]?.["sub_title"]?.["includes"]("banner") || json.data?.[0]?.["title"]?.["includes"]("banner")) {
        delete json.data[0].list_ads;
        delete json.data[0].list;
        $.log("去除页面广告");
      } else {
        if (json.data?.["banner"] || json.data?.["banners"]) {
          delete json.data.banners;
          $.log("去除页面广告");
        } else {
          if (json.data?.["activityData"]) {
            delete json.data.banner;
            delete json.data.activityData;
            $.log("去除页面广告");
          } else {
            if (json.data?.["detail"]) {
              json.data.ads = [];
              json.data.detail.coins = 0;
              json.data.detail.is_buy = true;
              $.log("解锁帖子");
            } else {
              if (json.data?.["previewUrl"]) {
                const regex = /^https:\/\/[^\/]*\.(.*\.cn)(\/\w+\/[a-z0-9]{32}\/[a-z0-9]{32}\.m3u8)(\?.*)?/;
                delete json.data.ads;
                json.data.hasBuy = true;
                json.data.coins = 0;
                const match = json.data.previewUrl.match(regex);
                json.data.previewUrl = "https://long." + match[1] + match[2] + match[3];
                json.data.playUrl = json.data.previewUrl;
                $.log("解锁视频");
              } else {
                if (json.data?.[0]?.[1]?.["playUrl"]) {
                  json.data.forEach(_0x3785b4 => _0x3785b4.coins = 0);
                  json.data.forEach(_0x292ab7 => _0x292ab7.hasBuy = true);
                  json.data.forEach(_0x3c16ce => {
                    typeof _0x3c16ce.playUrl === "string" && (_0x3c16ce.playUrl = _0x3c16ce.playUrl.replace(/\/\/[^.]+/, "//long"));
                  });
                } else {
                  if (json.data?.["items"]) {
                    json.data.items.forEach(_0x2d7b34 => _0x2d7b34.coins = 0);
                    json.data.items.forEach(_0x9e9ff7 => _0x9e9ff7.hasBuy = true);
                    json.data.items.forEach(_0xfa0672 => {
                      if (typeof _0xfa0672.playUrl === "string") {
                        {
                          _0xfa0672.playUrl = _0xfa0672.playUrl.replace(/\/\/[^.]+/, "//long");
                        }
                      }
                    });
                  } else {
                    if (json.data?.["list"]) {
                      json.data.list.forEach(_0x3d514d => _0x3d514d.coins = 0);
                      json.data.list.forEach(_0xee66b6 => _0xee66b6.hasBuy = true);
                      json.data.list.forEach(_0xb30e0b => {
                        typeof _0xb30e0b.playUrl === "string" && (_0xb30e0b.playUrl = _0xb30e0b.playUrl.replace(/\/\/[^.]+/, "//long"));
                      });
                    } else {
                      isArrayWithEmptyObject(json.data) ? (json.data.forEach(_0x4d31c7 => _0x4d31c7.coins = 0), json.data.forEach(_0x48262b => _0x48262b.hasBuy = true), json.data.forEach(_0x1c96b6 => {
                        if (typeof _0x1c96b6.playUrl === "string") {
                          {
                            _0x1c96b6.playUrl = _0x1c96b6.playUrl.replace(/\/\/[^.]+/, "//long");
                          }
                        }
                      })) : $.done({
                        body: body
...
json.isVip = true;
json.hasLogin = true;
data.data = encryptAES_CFB(JSON.stringify(json), key, iv);
data.timestamp = Math.floor(Date.now() / 1000);
let str = "data=" + data.data + "&errcode=0&timestamp=" + data.timestamp + "cc88ddc9357ff461e08f047aedee692b";
data.sign = S2(str);
$.done({
  body: JSON.stringify(data)
});
function isArrayWithEmptyObject(_0x3b0ac9) {
  return Array.isArray(_0x3b0ac9) && _0x3b0ac9.length === 1 && typeof _0x3b0ac9[0] === "object" && _0x3b0ac9[0] !== null && Object.keys(_0x3b0ac9[0]).length === 0;
}
function decryptAES_CFB(_0x2b57dd, _0x4092a1, _0x3fda48) {
  try {
    {
      const _0x5c968a = CryptoJS.enc.Hex.parse(_0x2b57dd);
      const _0x403bc6 = CryptoJS.enc.Base64.stringify(_0x5c968a);
      const _0x5733b8 = CryptoJS.AES.decrypt(_0x403bc6, _0x4092a1, {
        iv: _0x3fda48,
        mode: CryptoJS.mode.CFB,
        padding: CryptoJS.pad.NoPadding
      });
      return _0x5733b8.toString(CryptoJS.enc.Utf8);
    }
  } catch (_0x5d06ad) {
    console.error("解密错误:", _0x5d06ad);
    throw _0x5d06ad;
  }
}
function encryptAES_CFB(_0x53655d, _0x4f15a5, _0x4dffbc) {
  try {
    const _0x4edef0 = CryptoJS.AES.encrypt(_0x53655d, _0x4f15a5, {
      iv: _0x4dffbc,
      mode: CryptoJS.mode.CFB,
      padding: CryptoJS.pad.NoPadding
    });
    return CryptoJS.enc.Hex.stringify(CryptoJS.enc.Base64.parse(_0x4edef0.toString())).toUpperCase();
  } catch (_0x290a8d) {
    console.error("加密错误:", _0x290a8d);
    throw _0x290a8d;
  }
}
function S2(_0x3c1546) {
  let _0x4c39e7 = CryptoJS.SHA256(_0x3c1546).toString();
  return CryptoJS.MD5(_0x4c39e7).toString();
}
function createCryptoJS() {
  ...
  var c = c || function (t, e) {
    ...
    var u = h.Utf8;
    s.BufferedBlockAlgorithm = a.extend({
      reset: function () {
      ...
      _process: function (e) {
        ...
        var a = o / (4 * s);
        var h = (a = e ? t.ceil(a) : t.max((0 | a) - this._minBufferSize, 0)) * s;
        var l = t.min(4 * h, o);
        if (h) {
          for (var f = 0; f < h; f += s) {
    ...
    var d = s.BufferedBlockAlgorithm;
    s.Hasher = d.extend({
      cfg: a.extend(),
      init: function (t) {
    ...
    t.enc.Base64 = {
      stringify: function (t) {
        ...
        for (var n = [], o = 0; o < r; o += 3) {
          for (var s = (e[o >>> 2] >>> 24 - o % 4 * 8 & 255) << 16 | (e[o + 1 >>> 2] >>> 24 - (o + 1) % 4 * 8 & 255) << 8 | e[o + 2 >>> 2] >>> 24 - (o + 2) % 4 * 8 & 255, a = 0; a < 4 && o + 0.75 * a < r; a++) {
    ...
    t.enc.Base64url = {
      stringify: function (t, e) {
        ...
        for (var o = [], s = 0; s < i; s += 3) {
          for (var a = (r[s >>> 2] >>> 24 - s % 4 * 8 & 255) << 16 | (r[s + 1 >>> 2] >>> 24 - (s + 1) % 4 * 8 & 255) << 8 | r[s + 2 >>> 2] >>> 24 - (s + 2) % 4 * 8 & 255, c = 0; c < 4 && s + 0.75 * c < i; c++) {
      ...
      parse: function (t, r) {
        undefined === r && (r = true);
        var i = t.length;
        var n = r ? this._safe_map : this._map;
  ...
  (function (t) {
    ...
    !function () {
      ...
      for (var i = 2, n = 0; n < 64;) {
        e(i) && (n < 8 && (s[n] = r(t.pow(i, 0.5))), a[n] = r(t.pow(i, 0.3333333333333333)), n++);
        i++;
      }
    }();
    var h = [];
      ...
      _doProcessBlock: function (t, e) {
        for (var r = this._hash.words, i = r[0], n = r[1], o = r[2], s = r[3], c = r[4], l = r[5], f = r[6], u = r[7], d = 0; d < 64; d++) {
          if (d < 16) {
            ...
            var p = h[d - 15];
            var _ = (p << 25 | p >>> 7) ^ (p << 14 | p >>> 18) ^ p >>> 3;
            var v = h[d - 2];
            var y = (v << 15 | v >>> 17) ^ (v << 13 | v >>> 19) ^ v >>> 10;
            h[d] = _ + h[d - 7] + y + h[d - 16];
          }
          var g = i & n ^ i & o ^ n & o;
          var B = (i << 30 | i >>> 2) ^ (i << 19 | i >>> 13) ^ (i << 10 | i >>> 22);
          var w = u + ((c << 26 | c >>> 6) ^ (c << 21 | c >>> 11) ^ (c << 7 | c >>> 25)) + (c & l ^ ~c & f) + a[d] + h[d];
          u = f;
          f = l;
          l = c;
          c = s + w | 0;
      ...
      _doProcessBlock: function (t, e) {
        for (var r = this._hash.words, i = r[0], n = r[1], o = r[2], s = r[3], c = r[4], l = r[5], f = r[6], u = r[7], d = i.high, p = i.low, _ = n.high, v = n.low, y = o.high, g = o.low, B = s.high, w = s.low, k = c.high, b = c.low, x = l.high, S = l.low, m = f.high, A = f.low, z = u.high, H = u.low, C = d, R = p, D = _, E = v, M = y, P = g, F = B, W = w, O = k, I = b, U = x, K = S, X = m, L = A, j = z, T = H, N = 0; N < 80; N++) {
          ...
          if (N < 16) {
            ...
            var J = G.high;
            var Q = G.low;
            var Y = (J >>> 1 | Q << 31) ^ (J >>> 8 | Q << 24) ^ J >>> 7;
            var $ = (Q >>> 1 | J << 31) ^ (Q >>> 8 | J << 24) ^ (Q >>> 7 | J << 25);
            var tt = h[N - 2];
            var et = tt.high;
            var rt = tt.low;
          ...
          var pt = C & D ^ C & M ^ D & M;
          var _t = R & E ^ R & P ^ E & P;
          var vt = (C >>> 28 | R << 4) ^ (C << 30 | R >>> 2) ^ (C << 25 | R >>> 7);
          var yt = (R >>> 28 | C << 4) ^ (R << 30 | C >>> 2) ^ (R << 25 | C >>> 7);
          var gt = (O >>> 14 | I << 18) ^ (O >>> 18 | I << 14) ^ (O << 23 | I >>> 9);
          var Bt = (I >>> 14 | O << 18) ^ (I >>> 18 | O << 14) ^ (I << 23 | O >>> 9);
          var wt = a[N];
          var kt = wt.high;
          var bt = wt.low;
          var xt = j + gt + ((ft = T + Bt) >>> 0 < T >>> 0 ? 1 : 0);
          var St = yt + _t;
          j = X;
          T = L;
          X = U;
      ...
      _doProcessBlock: function (t, e) {
        ...
        for (var c = 0; c < 24; c++) {
          ...
          for (var k = 1; k < 25; k++) {
            ...
            var x = H.low;
            var S = a[k];
            S < 32 ? (d = b << S | x >>> 32 - S, p = x << S | b >>> 32 - S) : (d = x << S - 32 | b >>> 64 - S, p = b << S - 32 | x >>> 64 - S);
            var m = f[h[k]];
            m.high = d;
            m.low = p;
          }
  ...
  (function (t) {
    ...
    var a = i.create([5, 14, 7, 0, 9, 2, 11, 4, 13, 6, 15, 8, 1, 10, 3, 12, 6, 11, 3, 7, 0, 13, 5, 10, 14, 15, 8, 12, 4, 9, 1, 2, 15, 5, 1, 3, 7, 14, 6, 9, 11, 8, 12, 2, 10, 0, 4, 13, 8, 6, 4, 1, 3, 11, 15, 0, 5, 12, 2, 13, 9, 7, 10, 14, 12, 15, 10, 4, 1, 5, 8, 7, 6, 2, 13, 14, 0, 3, 9, 11]);
    var h = i.create([11, 14, 15, 12, 5, 8, 7, 9, 11, 13, 14, 15, 6, 7, 9, 8, 7, 6, 8, 13, 11, 9, 7, 15, 7, 12, 15, 9, 11, 7, 13, 12, 11, 13, 6, 7, 14, 9, 13, 15, 14, 8, 13, 6, 5, 12, 7, 5, 11, 12, 14, 15, 14, 15, 9, 8, 9, 14, 5, 6, 8, 6, 5, 12, 9, 15, 5, 11, 6, 8, 13, 12, 5, 12, 13, 14, 11, 8, 5, 6]);
    ...
    var u = i.create([1352829926, 1548603684, 1836072691, 2053994217, 0]);
    o.RIPEMD160 = n.extend({
      _doReset: function () {
  ...
  c.lib.Cipher || function (t) {
    ...
    (e.format = {}).OpenSSL = {
      ...
      parse: function (t) {
        ...
        var r = a.parse(t);
        var i = r.words;
        1398893684 == i[0] && 1701076831 == i[1] && (e = n.create(i.slice(2, 4)), i.splice(0, 4), r.sigBytes -= 16);
        return _.create({
          ciphertext: r,
          salt: e
        });
    ...
    var v = (e.format = {}).OpenSSL;
    r.SerializableCipher = i.extend({
      cfg: i.extend({
        format: v
      }),
    ...
    var g = (e.kdf = {}).OpenSSL;
    r.PasswordBasedCipher = y.extend({
      cfg: y.cfg.extend({
        kdf: g
      }),
      encrypt: function (t, e, r, i) {
  ...
  c.mode.CFB = function () {
    ...
    function e(t, e, r, i) {
      var n;
      var o = this._iv;
      o ? (n = o.slice(0), this._iv = undefined) : n = this._prevBlock;
      i.encryptBlock(n, 0);
      for (var s = 0; s < r; s++) {
        t[e + s] ^= n[s];
      }
  ...
  c.mode.CTR = (o = c.lib.BlockCipherMode.extend(), s = o.Encryptor = o.extend({
    processBlock: function (t, e) {
      ...
      var n = this._iv;
      var o = this._counter;
      n && (o = this._counter = n.slice(0), this._iv = undefined);
      var s = o.slice(0);
      r.encryptBlock(s, 0);
      o[i - 1] = o[i - 1] + 1 | 0;
      for (var a = 0; a < i; a++) {
  ...
  c.mode.CTRGladman = function () {
    ...
    t.Encryptor = t.extend({
      processBlock: function (t, r) {
        ...
        var o = this._iv;
        var s = this._counter;
        o && (s = this._counter = o.slice(0), this._iv = undefined);
        (function (t) {
  ...
  c.mode.OFB = function () {
    ...
    t.Encryptor = t.extend({
      processBlock: function (t, e) {
        ...
        n && (o = this._keystream = n.slice(0), this._iv = undefined);
        r.encryptBlock(o, 0);
        for (var s = 0; s < i; s++) {
          t[e + s] ^= o[s];
        }
    ...
    r.AES = e.extend({
      _doReset: function () {
        if (!this._nRounds || this._keyPriorReset !== this._key) {
          for (var t = this._keyPriorReset = this._key, e = t.words, r = t.sigBytes / 4, n = 4 * ((this._nRounds = r + 6) + 1), o = this._keySchedule = [], s = 0; s < n; s++) {
            s < r ? o[s] = e[s] : (h = o[s - 1], s % r ? r > 6 && s % r == 4 && (h = i[h >>> 24] << 24 | i[h >>> 16 & 255] << 16 | i[h >>> 8 & 255] << 8 | i[255 & h]) : (h = i[(h = h << 8 | h >>> 24) >>> 24] << 24 | i[h >>> 16 & 255] << 16 | i[h >>> 8 & 255] << 8 | i[255 & h], h ^= p[s / r | 0] << 24), o[s] = o[s - r] ^ h);
          }
      ...
      _doCryptBlock: function (t, e, r, i, n, o, s, a) {
        for (var c = this._nRounds, h = t[e] ^ r[0], l = t[e + 1] ^ r[1], f = t[e + 2] ^ r[2], u = t[e + 3] ^ r[3], d = 4, p = 1; p < c; p++) {
          ...
          var v = i[l >>> 24] ^ n[f >>> 16 & 255] ^ o[u >>> 8 & 255] ^ s[255 & h] ^ r[d++];
          var y = i[f >>> 24] ^ n[u >>> 16 & 255] ^ o[h >>> 8 & 255] ^ s[255 & l] ^ r[d++];
          var g = i[u >>> 24] ^ n[h >>> 16 & 255] ^ o[l >>> 8 & 255] ^ s[255 & f] ^ r[d++];
          h = _;
          l = v;
          f = y;
          u = g;
  ...
  (function () {
    ...
    var a = [1, 2, 4, 6, 8, 10, 12, 14, 15, 17, 19, 21, 23, 25, 27, 28];
    var h = [{
      0: 8421888,
      268435456: 32768,
      536870912: 8421378,
    ...
    n.TripleDES = i.extend({
      _doReset: function () {
        ...
        var n = t.length < 6 ? t.slice(0, 2) : t.slice(4, 6);
  ...
  (function () {
    ...
    r.Rabbit = e.extend({
      _doReset: function () {
        ...
        this._X = [t[0], t[3] << 16 | t[2] >>> 16, t[1], t[0] << 16 | t[3] >>> 16, t[2], t[1] << 16 | t[0] >>> 16, t[3], t[2] << 16 | t[1] >>> 16];
        var i = this._X;
        this._C = [t[2] << 16 | t[2] >>> 16, 4294901760 & t[0] | 65535 & t[1], t[3] << 16 | t[3] >>> 16, 4294901760 & t[1] | 65535 & t[2], t[0] << 16 | t[0] >>> 16, 4294901760 & t[2] | 65535 & t[3], t[1] << 16 | t[1] >>> 16, 4294901760 & t[3] | 65535 & t[0]];
        var n = this._C;
        this._b = 0;
        for (r = 0; r < 4; r++) {
          a.call(this);
        ...
        if (e) {
          ...
          var s = o[0];
          var c = o[1];
          var h = 16711935 & (s << 8 | s >>> 24) | 4278255360 & (s << 24 | s >>> 8);
          var l = 16711935 & (c << 8 | c >>> 24) | 4278255360 & (c << 24 | c >>> 8);
          var f = h >>> 16 | 4294901760 & l;
          var u = l << 16 | 65535 & h;
          n[0] ^= h;
          n[1] ^= f;
          n[2] ^= l;
    ...
    function a() {
      ...
      for (r = 0; r < 8; r++) {
        ...
        var h = ((4294901760 & i) * i | 0) + ((65535 & i) * i | 0);
        o[r] = c ^ h;
      }
  ...
  (function () {
    ...
    var r = t.algo;
    const i = 16;
    const n = [608135816, 2242054355, 320440878, 57701188, 2752067618, 698298832, 137296536, 3964562569, 1160258022, 953160567, 3193202383, 887688300, 3232508343, 3380367581, 1065670069, 3041331479, 2450970073, 2306472731];
...
function Env(a, b) {
  ...
  return new class {
    ...
    lodash_set(a = {}, b = "", c) {
      Array.isArray(b) || (b = this.toPath(b));
      b.slice(0, -1).reduce((a, c, d) => Object(a[c]) === a[c] ? a[c] : a[c] = /^d+$/.test(b[d + 1]) ? [] : {}, a)[b[b.length - 1]] = c;
      return a;
    }
    toPath(a) {
    ...
    async fetch(a = {}, b = {}) {
      ...
      a.method || (a.method = a.body ?? a.bodyBytes ? "POST" : "GET");
      ...
      switch (this.platform()) {
        ...
        default:
          ...
          a.followRedirect && ((this.isSurge() || this.isLoon()) && (a["auto-redirect"] = false), this.isQuanX() && (a.opts ? a.opts.redirection = false : a.opts = {
            redirection: false
          }));
          a.bodyBytes && !a.body && (a.body = a.bodyBytes, delete a.bodyBytes);
          return await new Promise((b, d) => {
            $httpClient[c](a, (c, e, f) => {
              c ? d(c) : (e.ok = /^2dd$/.test(e.status), e.statusCode = e.status, f && (e.body = f, true == a["binary-mode"] && (e.bodyBytes = f)), b(e));
            });
          });
        case "Quantumult X":
        ...
        switch (typeof a) {
          ...
          case "object":
            switch (this.platform()) {
              ...
              case "Quantumult X":
                {
                  let b = a["open-url"] || a.url || a.openUrl;
                  let c = a["media-url"] || a.mediaUrl;
                  let d = a["update-pasteboard"] || a.updatePasteboard;
                  return {
                    "open-url": b,
                    "media-url": c,
                    "update-pasteboard": d
    ...
    wait(a) {
      ...
      switch (this.log("", "🔔" + this.name + ", 结束! 🕛" + c + "秒"), this.platform()) {
        ...
        case "Quantumult X":
          ...
          delete a.path;
          delete a.policy;
          delete a["policy-descriptor"];
          delete a.scheme;
          delete a.sessionIndex;
          delete a.statusCode;
          delete a.timeout;
          a.body instanceof ArrayBuffer ? (a.bodyBytes = a.body, delete a.body) : ArrayBuffer.isView(a.body) ? (a.bodyBytes = a.body.buffer.slice(a.body.byteOffset, a.body.byteLength + a.body.byteOffset), delete a.body) : a.body && delete a.bodyBytes;
          $done(a);
      }
    }
  }(a, b);
